...
The repository includes only a small ISRUC demonstration subset. Access to the original datasets is governed by their respective providers and licenses; users should obtain each dataset from its official source.

For preprocessing choices, channel selection, sampling-rate normalization, segment duration, and split protocols used in the reported experiments, refer to the [paper](https://doi.org/10.1002/aisy.70486).
